...
document.querySelectorAll(".edit-table-button").forEach(button => {
    button.addEventListener("click", () => {
        ...
        document.getElementById("tableNotes").value = button.dataset.notes || "";
    });
});
...
document.querySelectorAll(".edit-signup-button").forEach(button => {
    button.addEventListener("click", () => {
        ...
        const marker = "Dietary Restrictions:";
        if (comment.includes(marker)) {
            const parts = comment.split(marker);
            cleanedComment = parts[0].trim();
            dietaryRestrictions = parts[1].trim();
        }

        form.action = `/drsu/edit_signup/${signupId}`; // set to edit route
        signupIdInput.value = signupId;
        dishInput.value = dish;
        dietaryRestrictionsInput.value = dietaryRestrictions;
        extrasInput.value = cleanedComment;
