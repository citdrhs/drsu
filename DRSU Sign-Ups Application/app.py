...
def create_app():
   ...
   # global global_app
    ...
    def homePage():
        ...
        cur.execute('SELECT t1.*,COALESCE(t2.first_name, \'\') as first_name, COALESCE(t2.last_name, \'\') as last_name FROM events AS t1 LEFT JOIN public.user AS t2 ON t2.email=t1.contact ORDER BY t1.date ASC;')
        events = cur.fetchall()
    ...
    def manage_admins():
        ...
        sort_by = request.args.get('sort', 'lastname_asc')
        ...
        if sort_by == 'firstname_asc':
            users = sorted(users, key=lambda u: u.first_name.lower())
        elif sort_by == 'firstname_desc':
            users = sorted(users, key=lambda u: u.first_name.lower(), reverse=True)
        elif sort_by == 'lastname_asc':
            users = sorted(users, key=lambda u: u.last_name.lower())
        elif sort_by == 'lastname_desc':
            users = sorted(users, key=lambda u: u.last_name.lower(), reverse=True)
        elif sort_by == 'email_asc':
            users = sorted(users, key=lambda u: u.email.lower())
        elif sort_by == 'email_desc':
            users = sorted(users, key=lambda u: u.email.lower(), reverse=True)
        elif sort_by == 'grade_asc':
            users = sorted(users, key=lambda u: (int(u.grade) if u.grade and str(u.grade).isdigit() else 999, u.last_name.lower(), u.first_name.lower()))
        elif sort_by == 'grade_desc':
            users = sorted(users, key=lambda u: (int(u.grade) if u.grade and str(u.grade).isdigit() else 999, u.last_name.lower(), u.first_name.lower()), reverse=True)
        else:  # Default sort by last name
            users = sorted(users, key=lambda u: (u.last_name.lower(), u.first_name.lower()))
